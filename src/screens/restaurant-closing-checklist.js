...
const RestaurantClosingChecklist = () => {
  ...
  useEffect(() => {
    const fetchChecklist = async () => {
      try {
        const docRef = doc(db, "checklists", "restaurant-closing");
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await setDoc(doc(db, "checklists", "restaurant-closing"), {
        title: "Daily Restaurant Closing Checklist",
        questions,
  ...
  return (
    <Box sx={{ maxWidth: 820, mx: "auto", py: 4, px: { xs: 2, md: 3 } }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: "16px", borderTop: `6px solid ${appTheme.colors.primary}`, background: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <Typography variant="h5" fontWeight="bold" color={appTheme.colors.primary}>Daily Restaurant Closing Checklist</Typography>
...
export default RestaurantClosingChecklist;
