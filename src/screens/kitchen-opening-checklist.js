...
const KitchenOpeningChecklist = () => {
  const [questions, setQuestions] = useState([defaultQuestion()]);
  const [loading, setLoading] = useState(false);
  const [fetching, setFetching] = useState(true);
  const [toast, setToast] = useState({ open: false, message: "", severity: "success" });

  // Load existing checklist from Firestore
  useEffect(() => {
    const fetchChecklist = async () => {
      try {
        const docRef = doc(db, "checklists", "kitchen-opening");
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await setDoc(doc(db, "checklists", "kitchen-opening"), {
        title: "Daily Kitchen Opening Checklist",
        questions,
    ...
    <Box sx={{ maxWidth: 820, mx: "auto", py: 4, px: { xs: 2, md: 3 } }}>
      ...
      <Paper
        ...
        sx={{
          ...
          mb: 3,
          borderRadius: "16px",
          borderTop: `6px solid ${appTheme.colors.primary}`,
          background: "#fff",
          boxShadow: "0 2px 12px rgba(0,0,0,0.08)",
        }}
      >
        <Typography variant="h5" fontWeight="bold" color={appTheme.colors.primary}>
          Daily Kitchen Opening Checklist
        </Typography>
        <Typography variant="body2" color="text.secondary" mt={0.5}>
          Add questions and choose the answer type for each. Users will fill this in the mobile app.
        </Typography>
      </Paper>
        ...
        >
          <CardContent sx={{ p: 3 }}>
            ...
            {q.type === "image" && (
              ...
              >
                <ImageIcon sx={{ color: "#aaa", fontSize: 32 }} />
                <Typography variant="body2" color="text.disabled" fontStyle="italic">
                  User will upload an image here...
                </Typography>
              </Box>
            )}
            ...
            {q.type === "time" && (
              ...
              >
                <Typography variant="body1" color="text.secondary" sx={{ letterSpacing: 2 }}>
                  -- : --
                </Typography>
                <Box flex={1} />
      ...
      <Paper
        ...
        sx={{
          ...
          mb: 3,
          borderRadius: "14px",
          border: `2px dashed ${appTheme.colors.primary}`,
          display: "flex",
          alignItems: "center",
      ...
      >
        ...
        <Typography fontWeight={600} color={appTheme.colors.primary}>
          Add New Question
        </Typography>
      </Paper>
      ...
      <Box display="flex" justifyContent="flex-end">
        <Button
          ...
          sx={{
            ...
            textTransform: "none",
            fontSize: "1rem",
            "&:hover": { backgroundColor: appTheme.colors.secondary },
          }}
        >
      ...
      >
        <Alert severity={toast.severity} onClose={() => setToast({ ...toast, open: false })} sx={{ borderRadius: "10px" }}>
          {toast.message}
        </Alert>
      </Snackbar>
    </Box>
...
export default KitchenOpeningChecklist;
